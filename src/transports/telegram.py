...
class TelegramTransport(Transport):
        ...
        for block in blocks:
            ...
            if block.is_text() or block.text:
                ...
                with tempfile.TemporaryFile(mode="r+b") as temp_file:
                    ...
                    temp_file.seek(0)
                    resp = requests.post(
                        url=f"{self.api_root}/sendPhoto?chat_id={chat_id}", files={"photo": temp_file}
                    )
                ...
                logging.error(f"Telegram transport unable to send a block of MimeType {block.mime_type}")
    ...
    def _parse_inbound(self, payload: dict, context: Optional[dict] = None) -> ChatMessage:
        ...
        if not isinstance(chat_id, int):
            raise SteamshipError(f"Bad 'chat_id' found in Telegram message: ({chat_id}). Should have been an int.")
        ...
        if not isinstance(message_id, int):
            raise SteamshipError(f"Bad 'message_id' found in Telegram message: ({message_id}). Should have been an int")
